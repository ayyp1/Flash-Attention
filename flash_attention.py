...
#forward pass implementation of the FA 
@triton.jit
def _attn_fwd_inner(
    O_block,
    l_i,
...
@triton.autotune(
    [
        ...
        for num_stages in ([2, 3])  # ponytail: 4+ overflows T4's 64KB shared mem at BLOCK_Q=128/BLOCK_KV=64; raise if targeting A100/H100.
...
def _attn_fwd(
    ...
    O_block = O_block / l_i[:, None] #normalize at the last instead of each block 
...
class TritonAttention(torch.autograd.Function): 
    ...
    def forward(ctx, Q, K, V, causal, softmax_scale): 
        ...
        ) #maximum of each row + logarithm of normalization factor we don't recompute on backward we just store and use this 
        ...
        _attn_fwd[grid](
            ...
            #strides are required for the pointer and creating the offsets 
    ...
    def backward(ctx, dO):
        ...
        BLOCK_SIZE_MICRO, BLOCK_SIZE_MACRO = 32, 64  # ponytail: MACRO=128 overflows T4's 64KB shared mem (K/V/dK/dV blocks scale with it); raise if targeting A100/H100.
...
if __name__ == "__main__":
    # ponytail: SEQ_LEN=4096 needs ~8.6GB just for the O(n^2) reference matrix — bump this
    # back up only on a GPU with enough headroom (the Triton kernel itself doesn't need it).
